...
def train(
    ...
    for i, batch in enumerate(pbar):
        ...
        for ix, chunk_json in enumerate(chunks):
            ...
            if B == 1: continue # skip batch size 1
            ...
            meta_cached_kvs = last_kv_meta_set.clone() if last_kv_meta_set != None else None
            cached_kv_lengths = torch.LongTensor([cached_kvs.shape[-2]] * cached_kvs.shape[2]).to(device) if cached_kvs != None else None
            meta_cached_kv_lengths = cached_kv_lengths.clone() if meta_cached_kvs != None else None
            ...
            if cur_selection_mask != None and cached_kvs != None:
                ...
                meta_cached_kvs = meta_cached_kvs[:,:,cur_selection_mask]
                meta_cached_kv_lengths = meta_cached_kv_lengths[cur_selection_mask]
            
            # ddp so add module
            out = model.module.meta_fwd_bwd(
                x = tokens, 
                targets = targets,
                loss_fn = loss_fn,
                length = lengths, 
                ...
                meta_cache = None if meta_cached_kvs == None else {
                    'cache': meta_cached_kvs,
                    'cache_lengths': meta_cached_kv_lengths
                }
            )
            cached_kvs = out['cache'] if 'cache' in out else None
            meta_cached_kvs = out['meta_cache'] if 'meta_cache' in out else None
            out_kvs = cached_kvs['cache']
            out_meta_kvs = meta_cached_kvs['cache']
            if max_cache_length != 0:
                last_kv_set = out_kvs[:, :, :, :, -max_cache_length:].clone()
                last_kv_meta_set = out_meta_kvs[:, :, :, :, -max_cache_length:].clone()

            
            loss, diff_loss = out["loss"], out["diff_loss"]
            ...
            last_kv_meta_set.detach_() if last_kv_meta_set != None else None

            full_loss = loss 
            full_loss /= cur_tokens_in_loss
            diff_loss_full = diff_loss / cur_tokens_in_loss
        
            
            print(f'loss: {full_loss}, diff_loss: {diff_loss_full}')
        ...
        del full_loss, diff_loss_full, cur_loss, cur_tokens_in_loss, loss
