...
class DynamicPositionBias(nn.Module):
    ...
    def __init__(self, dim, *, heads, depth, log_distance=False, norm=False, activation=nn.SiLU):
        ...
        self.mlp.append(nn.Sequential(
            nn.Linear(1, dim),
            DEFAULT_NORM(dim) if norm else nn.Identity(),
            activation()
        ))
...
class transformer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.temperature = nn.Parameter(torch.tensor(
            temperature), requires_grad=True) if shared_temperture else temperature
    ...
    def forward(self, x, length=None, self_condtioning=None, cache=None):
        ...
        mask, attn_mask, total_lens, x_len, cache_len, pos_bias = self.create_masks_and_positions(
            x, length, cache)

        cache_indices = self.get_cache_indices(x_len, cache_len, cache['cache'], x) if exists(
            cache) and self.cache_needs_gather else None

        for i, (attn, ff) in enumerate(self.layers):

            x = self.token_shift(x)
            a_out, kv = self.checkpoint(
                i, attn, x, pos_bias, attn_mask, self.get_cache(cache, layer=i), cache_indices)
            x = a_out + x
        ...
        if len(intermediate_logits) > 0:  # stack intermediate logits
            intermediate_logits = torch.stack(
                intermediate_logits, dim=0)  # D x B x N x L

        cached_kvs = torch.stack(cached_kvs, dim=0) if len(
            cached_kvs) > 0 else None
        cached_kvs = {'cache_lengths': total_lens,
                      'cache': cached_kvs} if exists(cached_kvs) else None

        # if the lengths are not the same, we need to gather the cache on the next forward pass (if cache is used)
...
class transformer_lm(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.abs_pos = lambda x: x + \
            self.abs_pos_fn(x) if self.use_abs_pos else x

        if self_conditioning:
            self.reprojection_layer = nn.Linear(vocab_size, dim)

        # if cosine sim is false set temp to normal transformer temp i.e. 1/sqrt(dim_head)
        if cosine_sim == False:
        ...
        self.to_logits = shared_embedding_output_layer(
            self.embedding) if self.tie_embedding else nn.Linear(dim, vocab_size)
    ...
    def self_condition_fn(self):
        def self_condition(x):
            ...
            # not effective for LMs (intermediate loss is tho)
...
def do_sample(distribution, temperature=1.0, top_k=25):
    if temperature == 0.0:
        ...
        if top_k != None: # set items below top_k to 0
            distribution = distribution.masked_fill(distribution < torch.topk(distribution, top_k)[0][..., -1, None], 0)
...
def greedy_generate(model, tokenizer, input_txt, max_len, force_cpu=False, temperature=0.0):
    ...
    model.to(device)
    input_ids = [0] + tokenizer.encode(input_txt)
    ...
    # print(output_tokens)
    return f'{tokenizer.decode(output_tokens)}'
...
def collate_fn(tensors: List[torch.Tensor], pad_token: int):  # only for testing!
    ...
    padded_tensors = [torch.cat([t, torch.full(
        (max_len - t.shape[0],), pad_token, dtype=t.dtype)], dim=0) for t in tensors]
...
def caching_test():
    ...
    s1_b1, s2_b1, s3_b1 = torch.tensor(tokenizer(
        '/hi')), torch.tensor(tokenizer('/buenos')), torch.tensor(tokenizer('/whats'))
    s1_b2, s2_b2, s3_b2 = torch.tensor(tokenizer(' there')), torch.tensor(
        tokenizer(' dias')), torch.tensor(tokenizer(' up'))
    s1_b3, s2_b3, s3_b3 = torch.tensor(tokenizer(' how')), torch.tensor(
        tokenizer(' captain')), torch.tensor(tokenizer(' donkey'))
    s1_b4, s2_b4, s3_b4 = torch.tensor(tokenizer(
        ' u/')), torch.tensor(tokenizer(' hook/')), torch.tensor(tokenizer(' man/'))
    b1, b1_lengths = collate_fn(
        [s1_b1, s2_b1, s3_b1], pad_token=tokenizer.token_to_id['#'])
    b2, b2_lengths = collate_fn(
        [s1_b2, s2_b2, s3_b2], pad_token=tokenizer.token_to_id['#'])
    b3, b3_lengths = collate_fn(
        [s1_b3, s2_b3, s3_b3], pad_token=tokenizer.token_to_id['#'])
    b4, b4_lengths = collate_fn(
        [s1_b4, s2_b4, s3_b4], pad_token=tokenizer.token_to_id['#'])
    # comparsion set final states of above should be the same as these
    f_1, f_2, f_3 = torch.tensor(tokenizer('/hi there how u/')), torch.tensor(tokenizer(
        '/buenos dias captain hook/')), torch.tensor(tokenizer('/whats up donkey man/'))
    fb, fb_lengths = collate_fn(
        [f_1, f_2, f_3], pad_token=tokenizer.token_to_id['#'])

    logits_s1, interim_logits, cached_kvs = model(b1, length=b1_lengths)
    logits_s2, interim_logits, cached_kvs_s2 = model(
        b2, length=b2_lengths, cache=cached_kvs)
    logits_s3, interim_logits, cached_kvs_s3 = model(
        b3, length=b3_lengths, cache=cached_kvs_s2)
    logits_s4, interim_logits, cached_kvs_s4 = model(
        b4, length=b4_lengths, cache=cached_kvs_s3)
    logits_fs, interim_logits, cached_kvs_fs = model(fb, length=fb_lengths)

    print('shapes: ', cached_kvs_fs['cache'].shape,
          cached_kvs_s4['cache'].shape)
    ...
    assert torch.allclose(
        fs_cache, cached_kvs_s4['cache'], atol=0.001), 'failed check ): ): ):'
