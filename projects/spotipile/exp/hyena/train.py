...
from lming.loading.datasets.MultiDataset import SimpleDataloader #####!
import traceback
    ...
    for i, batch in enumerate(pbar):
        ...
        for ix, chunk_json in enumerate(chunks):
            ...
            with autocast(device.type, dtype=torch.bfloat16):
                mask_to_pass = mask if lengths.max() != lengths.min() else None
                pred = model(x = tokens, mask=mask_to_pass)
                B,N,C = pred.shape 
